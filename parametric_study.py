...
def to_latex(data: Path, single: bool = False):
    ...
    latex_table = df2.to_latex(
        ...
        formatters={
            ...
            "Torque": f_format,
            "TEAM 30": f_format,
            "Relative Err": e_format,
            "Torque (Arkkio)": f_format,
    ...
    df2 = df2.rename(
        columns={
            "Speed": r"$\omega$",
            "Rotor_loss": "TEAM 30 (rotor)",
            "Steel_loss": "TEAM 30 (steel)",
        }
    )
    df2["Loss (rotor)"] = app["Rotor_loss"]
    df2["Relative Err (rotor)"] = abs(
        (df2["TEAM 30 (rotor)"] - df2["Loss (rotor)"]) / df2["TEAM 30 (rotor)"]
    )
    df2["Loss (steel)"] = app["Steel_loss"]
    df2["Relative Err (steel)"] = abs(
        (df2["TEAM 30 (steel)"] - df2["Loss (steel)"]) / df2["TEAM 30 (steel)"]
    )
    ...
    latex_table = df2.to_latex(
        ...
        formatters={
            r"$\omega$": i_format,
            "Loss (rotor)": f_format,
            "TEAM 30 (rotor)": f_format,
            "Relative Err (rotor)": e_format,
            "Loss (steel)": f_format,
            "TEAM 30 (steel)": f_format,
            "Relative Err (steel)": e_format,
        },
        position="!ht",
    ...
    latex_table = df2.to_latex(
        columns=[r"$\omega$", "TEAM 30", "Induced Voltage", "Relative Error"],
        index=False,
        escape=False,
        formatters={
            r"$\omega$": i_format,
            "TEAM 30": f_format,
            "Induced Voltage": f_format,
            "Relative Error": e_format,
        },
        position="!ht",
        column_format="ccccccc",
        caption=caption,
...
def create_plots(outdir: Path, outfile: str):
    ...
    txt = (
        r"Torque averaged over the period $t\in[$"
        + f"{T_min:.3e}, {T_max:.3e}"
        + r"] with "
        + f"dt={dt:.3e}."
    )
    ...
    txt = (
        r"RMS Voltage for Phase A and -A over the period $t\in[$"
        + f"{T_min:.3e}, {T_max:.3e}"
        + r"] with "
        + f"dt={dt:.3e}."
    )
    ...
    txt = (
        r"Power dissipation in the rotor over the period $t\in[$"
        + f"{T_min:.3e}, {T_max:.3e}"
        + r"] with "
        + f"dt={dt:.3e}."
    )
    ...
    plt.grid()
    txt = (
        r"Power dissipation in the steel rotor over the period $t\in[$"
        + f"{T_min:.3e}, {T_max:.3e}"
        + r"] with "
        + f"dt={dt:.3e}."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_phases", dest="num_phases", type=int, default=6, help="Number of phases"
    )
    parser.add_argument(
        "--steps",
        dest="steps",
